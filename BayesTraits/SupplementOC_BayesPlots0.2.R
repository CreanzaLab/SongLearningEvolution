...
plotBTjacks <- function(MateParam, SongParam, d, sigonly = FALSE, familysplit = NULL, nsim = 10) {
  ...
  if (SongParam == "Interval") {
    dfforminmax$songcontvec[which(dfforminmax$songcontvec == 0.01)] <- 0.25  #for visualization only
  }
  ...
  #   csvfiletemp <- csvfilevec[k]
    transitionplots(MateParam, SongParam, df = dfforminmax,newpdf = FALSE, sigonly = sigonly, minvec = minvec, maxvec = maxvec, nsim=nsim, familysplit = familysplit)
  # }
  # dev.off()
}
...
pdf(file=paste(Sys.Date(),MateParam,SongParam,"Bayes4plots.pdf",sep=""), width = 9, height = 8)
...
for (m in 1:length(segmentmeansdf$q12)) {
  ...
  text(x=c(50, 50, 94, 66, 50, 50, 6, 34), y=c(93, 67, 50, 50, 7,33, 50, 50), labels=labs, cex=0.4)
  #end plot
  
} #end for (m in 1:length(segmentmeans$q12)), i.e. end of going through each segment

MateSongParams = NULL
par(mar=c(4,3,2,0.5),cex.axis = 0.6, cex.lab = 0.6)
if (newpdf == TRUE) {
...
with(df, plot(songcontvec, LRpval, pch=20, 
              main = paste(MateSongParams, " Removed: ",familysplit,"; Original threshold range: ", round(min(alldfvals),4)," - ",round(max(alldfvals),3)," (",length(alldfvals),")", sep = ""), xlim = c(min(songvals)-0.1*min(songvals),max(songvals)+0.1*max(songvals)), log = "x", xlab = "", ylab="", cex.main = 0.48
))
