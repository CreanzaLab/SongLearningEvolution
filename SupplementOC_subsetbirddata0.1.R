...
if (newdata == FALSE) {
  alldatadfos <- as.data.frame(read.csv("OCPaperDataAll.csv"))
} else {alldatadfos <- as.data.frame(read.csv(newdata))}

if (newtree == FALSE) {
  matezillatree <- read.nexus("OCtreeHack1000.nex")
}
...
if (MateParam == "Polygyny") {
  matedf <- alldatadf[!is.na(alldatadf$Polygyny.final),]
  ...
  matecol <- "Polygyny.final"
} else if (MateParam == "EPP") {
  matedf <- alldatadf[!is.na(alldatadf$EPP.final),] 
  matecol <- "EPP.final"
  havedatavec <- matezillatree$tip.label %in% as.character(matedf$BirdtreeFormat)
  matingtips <- which(havedatavec == TRUE)
  dropformating <- which(havedatavec == FALSE)
